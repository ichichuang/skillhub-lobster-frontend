 ...
 * pagination and narrow keyword/status/hidden/label filters, plus the
 * owner-unscoped detail and version-file reads that back the admin skill
 * detail modal.
 */
@Service
public class AdminSkillAppService {
    ...
    public PageResponse<AdminSkillSummaryResponse> listSkills(String keyword,
        ...
        Page<Skill> skillPage = adminSkillQueryRepository.findSkills(
                keyword, resolvedStatus, hidden, resolvedLabel, PageRequest.of(page, size));
    ...
    public List<SkillFileResponse> listVersionFiles(Long skillId, Long versionId) {
        ...
        if (!version.getSkillId().equals(skill.getId())) {
            throw new DomainNotFoundException("error.skill.version.notFound", versionId);
        }
