 ...
 * state — including hidden, archived, and version-less skills that public
 * listing endpoints structurally exclude.
 */
@SpringBootTest
...
class AdminSkillListIntegrationTest {
    ...
    void listSkills_superAdmin_seesCrossOwnerSkillsIncludingHiddenWithOwnerFields() throws Exception {
        ...
        String slugVisible = createSkill(namespace, "visible", "owner-a-" + suffix, false, false, SkillStatus.ACTIVE, true);
        String slugHidden = createSkill(namespace, "hidden", "owner-b-" + suffix, true, false, SkillStatus.ACTIVE, false);
    ...
    void listSkills_paginationSplitsResultsWithoutOwnerScoping() throws Exception {
        ...
        createOwner("owner-a-" + suffix, "Owner A");
        createOwner("owner-b-" + suffix, "Owner B");
        String slugFirst = createSkill(namespace, "page1", "owner-a-" + suffix, false, false, SkillStatus.ACTIVE, false);
        createSkill(namespace, "page2", "owner-b-" + suffix, true, false, SkillStatus.ACTIVE, false);
        createSkill(namespace, "page3", "owner-a-" + suffix, false, false, SkillStatus.ACTIVE, false);
    ...
    void listSkills_hiddenFilter_returnsOnlyMatchingState() throws Exception {
        ...
        createOwner("owner-a-" + suffix, "Owner A");
        createOwner("owner-b-" + suffix, "Owner B");
        createSkill(namespace, "keep", "owner-a-" + suffix, false, false, SkillStatus.ACTIVE, false);
        String slugHidden = createSkill(namespace, "gone", "owner-b-" + suffix, true, false, SkillStatus.ACTIVE, false);
    ...
    @Test
    void listSkills_statusFilter_supportsActiveAndArchived() throws Exception {
        String suffix = uniqueSuffix();
        Namespace namespace = createNamespace(suffix);
        createOwner("owner-a-" + suffix, "Owner A");
        String slugActive = createSkill(namespace, "live", "owner-a-" + suffix, false, false, SkillStatus.ACTIVE, false);
        String slugArchived = createSkill(namespace, "old", "owner-a-" + suffix, false, false, SkillStatus.ARCHIVED, false);
        ...
        mockMvc.perform(get("/api/v1/admin/skills")
                ...
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.data.total").value(1))
                .andExpect(jsonPath("$.data.items[0].slug").value(slugArchived))
        ...
        mockMvc.perform(get("/api/v1/admin/skills")
                ...
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.data.total").value(1))
                .andExpect(jsonPath("$.data.items[0].slug").value(slugActive));
    ...
    @Test
    void listSkills_withoutSuperAdminRole_returns403() throws Exception {
        PlatformPrincipal principal = new PlatformPrincipal(
                "user-1", "user", "user@example.com", "", "session", Set.of("USER"));
    ...
    void listSkills_returnsAssignedLabelsAndEmptyCollectionForUnlabeledSkills() throws Exception {
        ...
        String slugLabeled = createSkill(namespace, "labeled", "owner-a-" + suffix, false, false, SkillStatus.ACTIVE, false);
        String slugUnlabeled = createSkill(namespace, "unlabeled", "owner-a-" + suffix, false, false, SkillStatus.ACTIVE, false);
    ...
    void listSkills_labelFilterMatchesAcrossOwnersAndIncludesEligibleHiddenSkills() throws Exception {
        ...
        String slugVisibleShared = createSkill(namespace, "vis-shared", "owner-a-" + suffix, false, false, SkillStatus.ACTIVE, false);
        String slugHiddenShared = createSkill(namespace, "hid-shared", "owner-b-" + suffix, true, false, SkillStatus.ACTIVE, false);
        String slugOther = createSkill(namespace, "other", "owner-a-" + suffix, false, false, SkillStatus.ACTIVE, false);
    ...
    void listSkills_labelFilterKeepsPaginationTotalsCorrect() throws Exception {
        ...
        createSkill(namespace, "p1", "owner-a-" + suffix, false, false, SkillStatus.ACTIVE, false);
        createSkill(namespace, "p2", "owner-a-" + suffix, false, false, SkillStatus.ACTIVE, false);
        createSkill(namespace, "p3", "owner-a-" + suffix, false, false, SkillStatus.ACTIVE, false);
    ...
    void listSkills_labelFilterComposesWithHiddenAndStatusFilters() throws Exception {
        ...
        createSkill(namespace, "combo-visible", "owner-a-" + suffix, false, false, SkillStatus.ACTIVE, false);
        String slugHiddenLabeled = createSkill(namespace, "combo-hidden", "owner-a-" + suffix, true, false, SkillStatus.ARCHIVED, false);
    ...
    @Test
    void listSkills_representsUnpublishedVersionlessSkillsForInspection() throws Exception {
        String suffix = uniqueSuffix();
        Namespace namespace = createNamespace(suffix);
        createOwner("owner-a-" + suffix, "Owner A");
        createSkill(namespace, "versionless", "owner-a-" + suffix, false, false, SkillStatus.ACTIVE, false);
        ...
        mockMvc.perform(get("/api/v1/admin/skills")
                ...
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.data.total").value(1))
                .andExpect(jsonPath("$.data.items[0].headlineVersion").value(nullValue()))
                .andExpect(jsonPath("$.data.items[0].publishedVersion").value(nullValue()))
                .andExpect(jsonPath("$.data.items[0].ownerPreviewVersion").value(nullValue()))
                .andExpect(jsonPath("$.data.items[0].resolutionMode").value("NONE"));
    }
    ...
    private String createSkill(Namespace namespace,
                               String marker,
                               String ownerId,
                               boolean hidden,
                               boolean archived,
                               SkillStatus status,
                               boolean withPublishedVersion) {
