...
class GovernanceWorkbenchAppServiceTest {
    ...
    @Test
    void listInbox_unknownExclusionValuesDoNotChangeResults() {
        ReviewTask reviewTask = createReviewTask(1L, 11L, 101L, "owner");
        PromotionRequest promotionRequest = createPromotionRequest(2L, 101L, 12L, "owner");
        ...
        when(governanceQueryRepository.getReportInboxItems(List.of()))
                .thenReturn(List.of());

        PageResponse<?> response = service.listInbox(
                "admin", Map.of(), Set.of("SKILL_ADMIN"), null, Set.of("FOO", "BAR"), 0, 20);

        assertThat(response.total()).isEqualTo(2);
        assertThat(response.items())
                .extracting(item -> ((GovernanceInboxItemResponse) item).type())
                .containsExactlyInAnyOrder("REVIEW", "PROMOTION");
    }
