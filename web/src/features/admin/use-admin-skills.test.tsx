...
import { useAdminSkills, useAdminSkillDetail, useHideSkill, useUnhideSkill, useHardDeleteSkill } from './use-admin-skills'
...
describe('useAdminSkills', () => {
  ...
  it('useAdminSkillDetail refetches when the selected skill id changes', async () => {
    getSkillDetailMock.mockReset().mockResolvedValue({
      skill: { id: 8, slug: 'next-skill', hidden: true, status: 'ACTIVE' },
      summary: '下一个技能',
      versions: [],
    })
    const { wrapper } = createWrapper()
    const { result, rerender } = renderHook(
      ({ skillId }: { skillId: number | null }) => useAdminSkillDetail(skillId, skillId !== null),
      { wrapper, initialProps: { skillId: 7 as number | null } },
    )
    ...
    await waitFor(() => expect(getSkillDetailMock).toHaveBeenCalledWith(8))
    await waitFor(() => expect(result.current.data?.skill.id).toBe(8))
  })
  ...
  it('useHardDeleteSkill removes the deleted detail/files caches and invalidates only list queries', async () => {
    ...
    expect(deleteSkillMock).toHaveBeenCalledWith(14)
    expect(hideSkillMock).not.toHaveBeenCalled()
  ...
  it('useHardDeleteSkill does not touch caches when the mutation fails', async () => {
    deleteSkillMock.mockRejectedValue(new Error('conflict'))
    const { wrapper, invalidateSpy, queryClient } = createWrapper()
