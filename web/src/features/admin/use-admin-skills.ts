...
/**
 * Upstream hook used by My Skills to restore a hidden skill. Kept verbatim:
 * it targets the ['skills'] query family and is unrelated to the admin
 * inventory keys below.
 */
...
// ---------------------------------------------------------------------------
// SUPER_ADMIN skill inventory (GET /api/v1/admin/skills family).
// ---------------------------------------------------------------------------
 ...
 *
 * Semantics note for maintainers: the UI wording 禁用/启用 maps strictly onto
 * the backend `hidden` governance overlay — 禁用 → adminApi.hideSkill,
 * 启用 → adminApi.unhideSkill. These hooks never touch skill lifecycle status
 * (ACTIVE/ARCHIVED) or version publication state. 删除技能 is a different
 * concept: useHardDeleteSkill maps onto the SUPER_ADMIN hard-delete endpoint
 * (DELETE /api/v1/skills/id/{skillId}), which permanently removes the skill,
 * its versions, files, and related data.
 */
...
export function useHardDeleteSkill() {
  const queryClient = useQueryClient()
  return useMutation({
    mutationFn: ({ skillId }: { skillId: number }) => adminApi.deleteSkillById(skillId),
