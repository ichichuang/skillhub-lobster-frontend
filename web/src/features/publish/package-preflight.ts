...
export type PreflightCheckId = 'paths' | 'skillMd' | 'frontmatter' | 'name' | 'description'
...
/**
 * Mirrors the server-side package path policy: an entry the official extractor
 * would reject makes the whole package invalid, so the preflight blocks it with
 * the offending path instead of silently skipping it.
 */
...
function buildNormalizedArchive(entries: ZipEntry[]): NormalizedArchive {
  ...
  const fileEntries = entries
    ...
    .map((entry) => {
      ...
      return { path: normalizedPath, entry }
    })
    .filter((item): item is { path: string; entry: ZipEntry } => item !== null)

  if (invalidPath) {
    return { pathByEntry: new Map(), ambiguousSkillMd: false, invalidPath }
  }
...
export async function runPackagePreflight(file: Blob): Promise<PackagePreflightResult> {
  ...
  const { pathByEntry, ambiguousSkillMd, invalidPath } = buildNormalizedArchive(archive.entries)
  ...
  if (invalidPath !== undefined) {
    checks.push({
      id: 'paths',
      labelKey: 'publish.preflight.pathsLabel',
      passed: false,
      failure: { messageKey: 'publish.preflight.pathInvalid', params: { path: invalidPath } },
    })
