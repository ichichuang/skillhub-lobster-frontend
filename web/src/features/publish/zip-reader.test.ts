...
describe('openZipArchive', () => {
  ...
  it('treats backslash-terminated entries as directory entries like the server extractor', async () => {
    const archive = await openZipArchive(storeBlob([
      { path: 'my-skill\\', content: '' },
      { path: 'my-skill/SKILL.md', content: skillMdContent('demo-skill', 'A demo skill') },
    ]))

    const windowsDirEntry = archive.entries.find((entry) => entry.path === 'my-skill\\')
    expect(windowsDirEntry?.isDirectory).toBe(true)
  })
