...
export function SkillCard({ skill, onClick, highlightStarred = true }: SkillCardProps) {
  ...
  return (
    <Card
      className="group relative h-full cursor-pointer overflow-hidden border bg-card p-5 text-card-foreground shadow-sm transition-shadow hover:shadow-md focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/70 focus-visible:ring-offset-2 focus-visible:ring-offset-background"
      onClick={onClick}
      onKeyDown={(event) => {
        if (!isInteractive) {
          return
