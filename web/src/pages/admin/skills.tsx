...
/**
 * After a permanent delete the server-side list shrank by one; step back if
 * the current page no longer exists instead of stranding the operator on an
 * empty page.
 */
export function computeLastValidPage(total: number, pageSize: number, page: number): number {
  const lastValidPage = Math.max(Math.ceil((total - 1) / pageSize) - 1, 0)
  return page > lastValidPage ? lastValidPage : page
}
...
/**
 * Task-oriented SUPER_ADMIN skill inventory page.
 *
...
export function AdminSkillsPage() {
  ...
  const confirmSkillDelete = async () => {
    ...
    try {
      ...
      setPage((current) => computeLastValidPage(total, PAGE_SIZE, current))
    } catch (error) {
      ...
      {isLoading ? (
        ...
        <>
          <Card className="flex min-h-0 flex-1 flex-col overflow-hidden">
            <div className="min-h-0 flex-1 overflow-x-hidden overflow-y-auto [&_td]:px-3 [&_th]:px-3">
