...
function CategoryBadge({ category }: { category: NotificationItem['category'] }) {
  ...
  const colorMap: Record<NotificationItem['category'], string> = {
    PUBLISH: 'bg-info-surface text-info',
    REVIEW: 'bg-warning-surface text-warning',
    PROMOTION: 'bg-success-surface text-success',
    REPORT: 'bg-danger-surface text-danger',
  }
  return (
                  ...
                  >
                    <span
                      className={`mt-2 h-2 w-2 flex-shrink-0 rounded-full ${
                        item.status === 'UNREAD' ? 'bg-danger' : 'bg-transparent'
                      }`}
                    />
